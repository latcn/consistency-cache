...
**核心特性**:
- ✅ **多级缓存支持**: L1(本地 Caffeine) + L2(分布式 Redis) 双层架构
- ✅ **一致性保障**: 支持 CP/AP 灵活切换，满足不同业务场景需求
- ✅ **热点检测**: 基于 Count-Min Sketch 的高性能热点检测，自动识别读写热点
- ✅ **可靠失效**: 基于 Redis Pub/Sub 和数据库事务发件箱模式的可靠失效广播
...
- ✅ **Hotspot Detection**: High-performance Count-Min Sketch based hot key detection
- ✅ **Reliable Invalidation**: Redis Pub/Sub combined with transactional outbox pattern
...
```sql
-- Execute script: script/db/hcc_cache_message.sql
