...
public class CacheMetricsBinder implements MeterBinder {
	...
	public CacheMetricsBinder(LocalCacheManager localCacheManager, DistributedCacheManager distributedCacheManager,
		...
		this(localCacheManager, distributedCacheManager, circuitBreaker, readHotspotDetector, writeHotspotDetector,
				null);
	}

	public CacheMetricsBinder(LocalCacheManager localCacheManager, DistributedCacheManager distributedCacheManager,
			CacheCircuitBreaker circuitBreaker, ReadHotspotDetector readHotspotDetector,
			WriteHotspotDetector writeHotspotDetector, SingleFlightExecutor singleFlightExecutor) {
	...
	private void bindLocalCacheMetrics() {
		...
		Gauge.builder("hcc_cache_hits_total", localCacheManager, manager -> manager.getStats().getHitCount())
			.description("Total number of cache hits")
			.tag("cache_level", "L1")
			.baseUnit("hits")
			.register(meterRegistry);

		Gauge.builder("hcc_cache_misses_total", localCacheManager, manager -> manager.getStats().getMissCount())
	...
	private void bindSingleFlightMetrics() {
		...
		Gauge.builder("singleflight_inflight_requests", singleFlightExecutor, executor -> executor.getInflightCount())
	...
	public void recordCacheGet(String cacheLevel, String result, long durationMs) {
		...
		getDurationTimers
			.computeIfAbsent(key,
					k -> Timer.builder("cache_get_duration")
