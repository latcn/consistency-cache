...
import io.github.latcn.cache.core.pubsub.BroadcastMessage;
 ...
 *
 * // 针对 ReliableTopic 的配置 config.setReliableTopicWatchdogTimeout(30 * 60 * 1000); //
 * 看门狗超时时间（毫秒），默认30分钟 // 如果客户端在此时间内未心跳，Redis会认为其离线，可能会清理其消费组（视具体策略而定）
 *
 * RedissonClient redisson = Redisson.create(config);
 *
 * RReliableTopic (基于 Stream) 1. 消息持久性：支持，消息存储在 Redis Stream 中，重连后可消费离线消息 2. 性能：
 * 较高，涉及磁盘写入和读取 3. 广播消息：支持，但每个实例需维护独立消费组 4. 适用场景: 订单处理、重要事件通知、需要可靠性的场景
 */
@Slf4j
	...
	public void broadcastMessage(Set<String> channelNames, BroadcastMessage message) {
		...
		for (String channelName : channelNames) {
			...
			topic.publishAsync(message).thenAccept(id -> {
				log.debug("缓存失效广播消息已发布，id:{}, message:{}", id, message);
			}).exceptionally(ex -> {
				log.error("缓存失效广播失败: {}, {}", ex.getMessage(), message);
				return null;
			});
		}
	}
