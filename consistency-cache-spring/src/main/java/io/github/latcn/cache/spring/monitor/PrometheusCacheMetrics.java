 ...
 * 
 * <p>设计意图：专门为Prometheus监控系统提供指标导出能力。Prometheus是目前最流行的
 * 云原生监控系统，通过此类可以方便地将缓存指标集成到Prometheus生态中。</p>
 * 
 * <p>主要功能：
 * <ul>
 ...
 * </p>
 * 
 * <p>指标格式：遵循Prometheus文本格式规范，可通过HTTP GET请求获取。</p>
 */
public class PrometheusCacheMetrics {
	 ...
	 * </p>
	 * 
	 * <p>JVM和系统指标的绑定使得监控系统能够全面了解应用的运行状态，
	 * 不仅包括缓存性能，还包括整体系统健康状况。</p>
	 * 
	 ...
	 * 获取Prometheus格式的指标数据。
	 * 
	 * <p>设计意图：提供Prometheus文本格式的指标导出接口，供Prometheus Server定期采集。
	 * 此方法应通过HTTP端点暴露，通常配置在/actuator/prometheus或/metrics路径。</p>
	 * 
	 * <p>返回格式：遵循Prometheus文本格式规范，包含所有已注册的指标及其当前值。</p>
	 * 
	 ...
	 * 
	 * <p>设计意图：提供对底层CollectorRegistry的直接访问，用于高级使用场景。
	 * CollectorRegistry是Prometheus Java客户端的核心组件，可以用于：
	 * <ul>
	 ...
	 * 获取缓存指标管理器。
	 * 
	 * <p>设计意图：提供对CacheMetricsManager的访问，用于：
	 * <ul>
