...
public class LocalCacheMarkerManagerImpl extends LocalCacheMarkerManager {
	...
	/**
	 * Lua 脚本：添加标记并自动续期 逻辑： 1. 移除已过期的节点 (Score < 当前时间) 2. 添加当前节点 3. 获取当前最大的 Score 4. 设置
	 * Key 的过期时间为 (最大Score + 缓冲时间)
	 */
	...
	/**
	 * Lua 脚本：删除标记 逻辑： 1. 移除已过期的节点 (Score < 当前时间) 2. 删除当前节点 3. 没有元素了，直接删除 Key 释放内存 4.
	 * 重置整个key的过期时间，避免漏删，redis key自动过期
	 */
	...
	/**
	 * Lua 脚本：主动清理过期节点并重置 TTL 逻辑： 1. 移除过期节点 2. 如果集合为空，删除 Key 3. 如果集合不为空，调整 TTL 为当前最大 Score
	 * + 缓冲时间
	 */
	...
	/**
	 * Lua 脚本：清理过期节点并返回活跃节点列表 逻辑： 1. 清理过期节点 2. 如果集合为空，删除 Key 3. 如果集合不为空，修正 TTL 4.
	 * 返回当前所有活跃节点
	 */
	...
	public void removeLocalCacheUsage(String cacheKey) {
		...
		String markerKey = MARKER_PREFIX + cacheKey;
		// 执行 Lua 脚本
		executeWithCache(SCRIPT_REMOVE_AND_RENEW_NAME, RScript.ReturnType.VALUE, Collections.singletonList(markerKey),
				new Object[] { this.nodeId, String.valueOf(System.currentTimeMillis()),
						String.valueOf(this.bufferTimeMs) });
	...
	public void doCleanUp() {
		try {
			...
			for (String markerKey : monitorKeys) {
				...
				RFuture<Long> rFuture = batch.getScript(StringCodec.INSTANCE)
					.evalShaAsync(RScript.Mode.READ_WRITE, cachedSha1, RScript.ReturnType.VALUE,
							Arrays.asList(markerKey), String.valueOf(now), String.valueOf(this.bufferTimeMs));
				useLocalCount.add(new RFutureWrapper(markerKey, rFuture));
			}
			batch.execute();
			for (RFutureWrapper<Long> rFuture : useLocalCount) {
				try {
	...
	public List<String> getActiveNodes(String cacheKey) {
		...
		List<Object> result = executeWithCache(SCRIPT_GET_ACTIVE_NODES_NAME, RScript.ReturnType.VALUE,
				...
				new Object[] { String.valueOf(now), String.valueOf(this.bufferTimeMs) });
		// 转换结果
		if (result == null) {
	...
	private <R> R executeWithCache(String scriptName, RScript.ReturnType returnType, List<Object> keys, Object[] args) {
		...
		try {
			...
			// 4. 容错处理：如果 Redis 重启或执行了 SCRIPT FLUSH，SHA1 会失效 捕获异常（Redis 通常返回 'NOSCRIPT'
			// 错误），重新加载脚本
	...
	class RFutureWrapper<T> {
		...
		public T get(long timeout) throws Exception {
			return this.rFuture.get(timeout, TimeUnit.SECONDS);
		}
