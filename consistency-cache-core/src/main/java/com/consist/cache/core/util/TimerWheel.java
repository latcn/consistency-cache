...
public class TimerWheel {
    ...
    public TimerWheel(int num, int interval) {
        this(num, interval, null);
    }

    public TimerWheel(int num, int interval, TimerWheel lowerWheel) {
        if (num<=0 || interval<=0) {
    ...
    private void init() {
        ...
        this.executor = new ThreadPoolExecutor(3, 3, 60,
                TimeUnit.SECONDS, new LinkedBlockingQueue<>(1024),
                r -> {
                    Thread thread = new Thread(r);
                    thread.setName("TimerWheel-Timer-Executor-"+threadCount.incrementAndGet());
    ...
    public void addTask(TimerTask timerTask) {
        ...
        if (wheelsIndex <= num) {
            ...
            overflowTimer.compareAndSet(null, new TimerWheel(num, interval*num, this));
