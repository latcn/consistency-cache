...
public class DefaultReadHotspotDetector implements ReadHotspotDetector {
	...
	/**
	 * Record a read operation for given key. Automatically triggers cleanup if interval
	 * exceeded.
	 * @param key cache key
	 */
	public <T> void recordRead(T key) {
		this.counters.computeIfAbsent(key, k -> new SlidingWindowCounter(windowSizeMs, bucketCount)).increment();
	}
	...
	/**
	 * Cleanup old entries to prevent memory leak. Automatically triggered during
	 * recordRead if interval exceeded.
	 */
	public void cleanup() {
		try {
			...
			this.counters.entrySet().removeIf(entry -> now - entry.getValue().lastAccessTime > COUNTER_TTL_MS);
			synchronized (this) {
