...
public class InvalidationRecordSqls {
	...
	protected static final Map<String, String> sqlMap = Map.ofEntries(
			...
			entry("getPendingCount", "SELECT COUNT(*) FROM " + INVALIDATION_RECORD_REPLACE + " WHERE status = 0"),
			entry("getFailedCount", "SELECT COUNT(*) FROM " + INVALIDATION_RECORD_REPLACE + " WHERE status = 2"));
