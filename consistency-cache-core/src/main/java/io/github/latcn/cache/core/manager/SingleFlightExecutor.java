 ...
 * 
 * <p>设计意图：当多个线程同时请求同一 key 的数据时，只让一个线程（leader）
 * 执行实际的数据加载操作，其他线程等待结果。这有效防止了缓存击穿问题，
 * 大幅降低后端数据库或分布式缓存的压力。</p>
 * 
 * <p>监控支持：通过 executeWithResult 方法返回 SingleFlightResult，
 * 调用方可以判断请求是否被去重，从而记录监控指标评估 SingleFlight 效果。</p>
 */
public class SingleFlightExecutor {
	 ...
	 * 
	 * <p>设计意图：让调用方能够知道当前请求是否被合并，从而决定是否记录监控指标。
	 * 这是评估 SingleFlight 效果的关键信息。</p>
	 * 
	 ...
	 */
	private <V> SingleFlightResult<V> waitForResult(CompletableFuture<SingleFlightResult<Object>> future, boolean deduplicated) {
		try {
