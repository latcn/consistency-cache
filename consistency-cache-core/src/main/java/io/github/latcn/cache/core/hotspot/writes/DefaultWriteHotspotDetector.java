...
import io.github.latcn.cache.core.util.RandomUtil;
 ...
 *
 * Features: - Sliding window invalidation counting - Automatic blacklist on threshold
 * breach - Exponential backoff for repeated violations - Auto-recovery with TTL
 */
@Slf4j
public class DefaultWriteHotspotDetector implements WriteHotspotDetector {
	...
	/**
	 * Record an L1 cache invalidation. Automatically detects and handles write hotspots.
	...
	private <T> void handleWriteHotspotDetected(T key, WriteHotSpotInfo info) {
		...
		log.warn("Write hotspot detected for key={}. Blacklisted for {}. Violation count: {}", key, blacklistDuration,
				info.violationCount);
	}
	...
	private Duration calculateBackoffDuration(WriteHotSpotInfo info) {
		...
		log.debug("Calculated backoff duration: {}ms (violations={}, multiplier={})", result.toMillis(),
				info.violationCount, multiplier);
	...
	/**
	 * Cleanup old entries to prevent memory leak. Only performs cleanup if interval has
	 * elapsed.
	 */
	public void cleanup() {
		try {
