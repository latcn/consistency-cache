 ...
 * 两级热点检测器（最终优化版）
 *
 * <h2>算法设计思路</h2> 采用“粗筛 + 精确跟踪”的两级架构，兼顾内存与准确性：
 * <ul>
 * <li><b>第一级（CMS 层）</b>：使用 Count-Min Sketch 存储所有 key 的粗略计数，内存固定。 作为“过滤器”，快速识别出可能成为热点的候选
 * key。</li>
 ...
 * <li><b>exactCounterDecayRate</b>：精确层每秒衰减率，建议 0.01～0.05。 越大则历史计数衰减越快，热点判定更偏向近期。</li>
...
public class TwoLevelHotKeyDetector implements AutoCloseable {
	...
	public static class Builder {
		...
		public static Builder forHighQps() {
			return new Builder().cmsWidth(1_000_000)
				.cmsDepth(3)
				.cmsDecayRate(0.15)
		...
		public Builder cmsWidth(int width) {
			this.cmsWidth = width;
			return this;
		}

		public Builder cmsDepth(int depth) {
			this.cmsDepth = depth;
			return this;
		}

		public Builder cmsDecayRate(double rate) {
			this.cmsDecayRate = rate;
			return this;
		}

		public Builder cmsDecayIntervalMs(long interval) {
			this.cmsDecayIntervalMs = interval;
			return this;
		}

		public Builder cmsSegmentCount(int count) {
			this.cmsSegmentCount = count;
			return this;
		}

		public Builder promotionThreshold(long threshold) {
			this.promotionThreshold = threshold;
			return this;
		}

		public Builder hotKeyThreshold(long threshold) {
			this.hotKeyThreshold = threshold;
			return this;
		}

		public Builder exactCounterDecayRate(double rate) {
			this.exactCounterDecayRate = rate;
			return this;
		}

		public Builder expirationTimeMs(long time) {
			this.expirationTimeMs = time;
			return this;
		}

		public Builder cleanupIntervalMs(long interval) {
			this.cleanupIntervalMs = interval;
			return this;
		}

		public Builder maxExactSize(int size) {
			this.maxExactSize = size;
			return this;
		}

		public Builder forceCleanupCooldownMs(long cooldown) {
			this.forceCleanupCooldownMs = cooldown;
			return this;
		}
		...
		public TwoLevelHotKeyDetector build() {
			if (promotionThreshold == null || hotKeyThreshold == null) {
				throw new CacheException(CacheError.INVALID_PARAMETER,
						"promotionThreshold and hotKeyThreshold must be set");
			}
			if (promotionThreshold <= 0 || hotKeyThreshold <= 0 || promotionThreshold >= hotKeyThreshold) {
				throw new CacheException(CacheError.INVALID_PARAMETER,
						"Invalid thresholds: promotionThreshold must be positive and less than hotKeyThreshold");
			}
	...
	private TwoLevelHotKeyDetector(Builder b) {
		this.cmsFilter = new CMSHotKeyDetector(b.cmsWidth, b.cmsDepth, b.cmsDecayRate, b.cmsDecayIntervalMs,
				b.cmsSegmentCount);
		...
		this.cleanupScheduler = Executors.newSingleThreadScheduledExecutor(r -> {
			...
			t.setUncaughtExceptionHandler((thread, ex) -> log.error("Cleanup thread died", ex));
			return t;
		});
	...
	public void setForceCleanupCooldownMs(long cooldownMs) {
		if (cooldownMs <= 0)
			throw new CacheException(CacheError.INVALID_PARAMETER, "cooldown must be positive");
	...
	/**
	 * 记录一次访问。 若 key 在精确层，直接更新精确计数（惰性衰减 +1）。 否则在 CMS 中记录，若 CMS 估计值达到晋升阈值，则尝试提升到精确层。
	 * 若精确层已满，尝试强制清理释放空间。
	 */
	public void record(String key) {
		if (closed)
			throw new CacheException(CacheError.HOT_KEY_DETECTION_FAILED, "TwoLevelHotKeyDetector already closed");
		if (key == null) {
	...
	/**
	 * 判断 key 是否为热点。 仅查询精确层，若不存在则返回 false。 查询时先对计数进行惰性衰减（但不增加），再与 hotKeyThreshold 比较。
	 */
	...
	public int getExactSize() {
		return exactCounter.size();
	}

	public long getForceCleanupCount() {
		return forceCleanupCount.get();
	}

	public long getForceCleanupSkipCooldown() {
		return forceCleanupSkipCooldown.get();
	}
	...
	private void forceCleanupIfNeeded(long nowNano) {
		...
		try {
			...
			if (!lastForceCleanupTime.compareAndSet(last, nowNano))
				return;
	...
	/**
	 * 强制清理：先执行常规清理，若仍超容量，则淘汰最低计数的 key。 为防止全量排序开销过大，当精确层条目 > 5000 时，随机采样 2000 个进行排序淘汰。
	...
	private void doCleanup(long nowNano) {
		exactCounter.entrySet().removeIf(entry -> {
			...
			long current = stats.getCount(exactCounterDecayRate, nowNano);
			return current < hotKeyThreshold
					&& (nowNano - stats.getLastAccessNano() > TimeUnit.MILLISECONDS.toNanos(expirationTimeMs));
		});
	}
	...
	/**
	 * 精确层每个 key 的状态：包含 double 计数和上次访问的纳秒时间戳。 采用无锁 AtomicReference<State> 实现原子更新。
	 *
	 * <p>
	 * 衰减策略：每次访问或查询时，根据上次衰减时间计算该段时间的衰减因子， 将历史计数乘以因子，再加上本次访问（+1）得到新计数。 使用 double
	 * 存储以避免小计数截断误差。
	 */
	private static class KeyStats {
		...
		/**
		 * 记录一次访问：读取旧状态，计算衰减，加 1，CAS 更新。 若间隔 > 1000 秒，直接归零（避免大数 Math.pow 开销）。
		 */
		...
		/**
		 * 仅获取当前衰减后的计数值（不加 1），用于查询和清理。 逻辑与 record 相同，但不加 1。
		 */
