...
/**
 * 广播、订阅初始化处理 生产者消费者模型： 为避免广播消息爆炸，间隔时间或累积一定量的消息后才统一发送 累积keys的消息大小对发送速率的影响
 */
public abstract class Broadcaster<T, S extends BroadcasterListener> {
	...
	public Broadcaster(BroadcastPublisher publisher, BroadcastSubscriber<T, S> subscriber,
			List<BroadcasterListener> listeners, int batchSize, int maxWaitSeconds) {
