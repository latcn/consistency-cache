...
class CacheKeyTest {
	...
	void testDefaultValues() {
		// When
		CacheKey<String> cacheKey = CacheKey.<String>builder().key("test-key").build();
	...
	void testDifferentKeyTypes() {
		// Given
		CacheKey<Integer> intKey = CacheKey.<Integer>builder().key(123).build();

		CacheKey<Long> longKey = CacheKey.<Long>builder().key(456789L).build();

		CacheKey<String> stringKey = CacheKey.<String>builder().key("string-key").build();
